...
export class ActivityDocumentRepository {
  ...
  private putContentInDossierFile(path: string) {
    return async (dossierFile: ActivityFile) => {
      return {
        ...dossierFile,
  ...
  async retrieveFileContentsInActivity(
    ...
    const newCertifications = (await Promise.all(document.certification.map(this.putContentInDossierFile(certificationsPath))))
      .map(f => new ActivityFile(f));
    const newEndorsements = (await Promise.all(document.endorsement.map(this.putContentInDossierFile(endorsementsPath))))
      .map(f => new ActivityFile(f));
    const newFiles = (await Promise.all(document.file.map(this.putContentInDossierFile(filesPath))))
      .map(f => new ActivityFile(f));
